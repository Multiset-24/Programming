...
int dp[105][100005];
bool rec(int i, int sum, vector<int>& a, int n) {
    if (sum < 0) return false; // If sum is negative, it's not possible
    if (i == n) return sum == 0; // If we've considered all elements, check if sum is zero

    if (dp[i][sum] != -1) return dp[i][sum]; // Return memoized result

    bool pick = rec(i + 1, sum - a[i], a, n);
    bool not_pick = rec(i + 1, sum, a, n);
