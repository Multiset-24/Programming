...
int dp[105][4];
int rec(int i,vector<int>&days,int last){
    if(i==days.size()){
        return 0;
    }
    if(last!=-1 && dp[i][last]!=-1) return dp[i][last];
    int rest=1+rec(i+1,days,0);

    if(days[i]==1){
        if(last!=1)rest=min(rest,rec(i+1,days,1));
    }
    if(days[i]==2){
        if(last!=2)rest=min(rest,rec(i+1,days,2));
    }
    if(days[i]==3){
        if(last!=1)rest=min(rest,rec(i+1,days,1));
        if(last!=2)rest=min(rest,rec(i+1,days,2));
