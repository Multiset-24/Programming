...
void jarvis()
{
    ...
    map<int, vector<int>> num_checker;

    for (int i = 0; i < n; i++)
    {
        num_checker[num[i]].push_back(i);
    }
    ...
    while (q--)
    {
        ...
        map<char, vector<int>> s_checker;

        for (int i = 0; i < n; i++)
        {
            s_checker[s[i]].push_back(i);
        }
        bool ans = true;
        for (auto it : num_checker)
        {
            char c = s[it.second[0]];
        ...
        for (auto it : s_checker)
        {
            int n = num[it.second[0]];
            for (auto idx : it.second)
            {
                if (n != num[idx])
                {
