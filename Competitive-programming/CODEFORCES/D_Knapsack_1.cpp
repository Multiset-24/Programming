...
int dp[105][100005];
int knapsack(int i,int W,vector<int>&weight,vector<int>&profit){
    if(W<0) return INT_MIN;

    if(W==0 || i==weight.size())return 0;
    if(dp[i][W]!=-1) return dp[i][W];
    int not_take=knapsack(i+1,W,weight,profit);
    int take=profit[i]+knapsack(i+1,W-weight[i],weight,profit);
...
void jarvis()
{
    ...
    cout<<knapsack(0,k,weight,profit);
}
