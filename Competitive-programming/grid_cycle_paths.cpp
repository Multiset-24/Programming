...
int res(int i, int j, int level) {
    ...
    if (s[level] == '?') {
        for (int k = 0; k < 4; k++) {
            ...
            if(r<0 || r>=7 || c<0 || c>=7 || maze[r][c])continue;
            cnt += res(r, c, level + 1);
        }
    }                                                                                                                                                                                                                                                                                                                                                                             
    else {
        if (s[level] == 'U') {
            cnt += (i-1<0 || maze[i-1][j])?0:res(i - 1, j, level + 1);
        }
        else if (s[level] == 'D') {
            cnt += (i+1>=7 || maze[i+1][j])?0:res(i + 1, j, level + 1);
        }
        else if (s[level] == 'L') {
            cnt += (j-1<0 || maze[i][j-1])?0:res(i, j - 1, level + 1);
        }
        else if (s[level] == 'R') {
            cnt += (j+1>=7 || maze[i][j+1])?0:res(i, j + 1, level + 1);
        }
    }
